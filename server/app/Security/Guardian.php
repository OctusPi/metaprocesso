...
class Guardian
{
    ...
    public static function checkAccess(string $module_name): bool
    {
        ...
        $user = self::getUser();
        if (!is_null($user)) {
            $module_name  = str_replace(['organs', 'units', 'sectors', 'ordinators', 'demandants','comissionsmembers','comissionsends', 'comissions', 'programs',  'dotations'], 'management', $module_name);
            $module_name  = str_replace(['catalogitems', 'suppliers'], 'catalogs', $module_name);
