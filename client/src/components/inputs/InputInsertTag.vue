...
<template>
    ...
    <div class="input-group">
        <input @keydown.enter.prevent="saveServiceIfUnique" type="text" :name="identify"
            class="form-control"
    ...
    <div class="d-flex flex-wrap gap-2 form-control my-2" :class="{ 'form-control-alert': valid }">
        <div class="tag d-flex gap-2" v-for="(item, i) in model" :key="i">
            <span>{{ item }}</span>
            <button @click="removeSevice(i)" type="button"
                class="btn m-0 p-0">
