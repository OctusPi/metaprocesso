    ...
    <div class="row position-relative">
        <div class="col-sm-12">
            ...
            <div class="input-group">
                ...
                <button class="btn btn-action-primary" type="button" @click="searchItems">
                    <ion-icon name="search" class="fs-5"></ion-icon>
                </button>
            </div>
        </div>

        <div v-if="component.search && (component.items || component.list)"
            class="search position-absolute mt-2 top-100 start-0">
            <div class="form-control load-items-cat p-0 m-0">
                <ul style="overflow: scroll;">
                    ...
                    <template v-if="component.selected?.tipo === 'M'">
                        <li v-for="l in component.items?.items" :key="l.codigoItem" @click="selectItem(l)"
                            class="d-flex align-items-center px-3 py-2">
                            <div class="me-3 item-type">{{ component.selected?.tipo }}</div>
                            <div class="item-desc">
                                <h3 class="m-0 p-0 small">{{ `${l.codigoItem} - ${l.nomePdm}` }}</h3>
                                <span class="m-0 p-0 me-2 small" v-for="c in l.buscaItemCaracteristica"
                                    :key="c.codigoCaracteristica">
                    ...
                    <template v-if="component.selected?.tipo === 'S'">
                        <li v-for="l in component.items?.items" :key="l.codigoServico" @click="selectItem(l)"
                            class="d-flex align-items-center px-3 py-2">
                            <div class="me-3 item-type">{{ component.selected?.tipo }}</div>
                            <div class="item-desc">
