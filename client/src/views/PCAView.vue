...
const exportCsv = (id) => {
	http.get(`${page.url}/export/${id}`, emit, (resp) => {
		const data_csv = json2csv(resp.data?.items)
		exp.exportCSV(data_csv, `pca-${id}`)
	})
}
					...
					<form @submit.prevent="pageData.list" class="row g-3">
						<div class="col-sm-12 col-md-4">
							...
							<input maxlength="4" type="text" name="s-reference_year" class="form-control"
								:class="{ 'form-control-alert': page.valids.reference_year }" id="s-reference_year"
								placeholder="AAAA" v-maska:[masks.masknumbs] v-model="page.search.reference_year" />
						</div>
						<div class="col-sm-12 col-md-4">
						...
						<div class="row m-0 mb-3 g-3 content p-4 pt-1">
							<div class="col-sm-12 col-md-4">
								...
								<input maxlength="4" type="text" name="reference_year" class="form-control"
									:class="{ 'form-control-alert': page.valids.reference_year }" id="reference_year"
									placeholder="AAAA" v-maska:[masks.masknumbs] v-model="page.data.reference_year" />
							</div>
							<div class="col-sm-12 col-md-4">
								...
								<div class="p-4 h-100 mb-5">
									<div class="chart-heading">
										<div class="d-flex gap-2 nav nav-tabs m-0 p-0 border-0">
											...
											<button type="button" class="btn nav-link p-0" data-bs-toggle="tab"
												data-bs-target="#origem">
												<span title="Origem">
													<ion-icon name="document" />
												</span>
											</button>
											<button type="button" class="btn nav-link p-0" data-bs-toggle="tab"
												data-bs-target="#adicionais">
											...
											<div id="origem" class="tab-pane w-100">
												<div class="row graph-row w-100">
													<div class="col-12 text-center mb-4">
														...
														<h1 class="m-0 txt-color">
															{{ utils.getTxt(page.selects.comissions,
																page.dfd.pca.comission_id) }}
														</h1>
													</div>
